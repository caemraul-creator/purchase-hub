...
// ======================
function loadData() {
  const old = document.getElementById('jsonp-rekap');
  if (old) old.remove();

  const s = document.createElement('script');
  s.id = 'jsonp-rekap';
  s.src = API_URL + '?sheet=done&callback=onRekapLoaded';
...
// CALLBACK JSONP
// ======================
function onRekapLoaded(data) {
  allData = data || [];
  filteredData = [...allData];

  headers = Object.keys(allData[0] || {})
    .filter(h => !REKAP_HIDDEN_COLUMNS.includes(h));

  currentPage = 1;
  // renderSummary();   <-- dihapus (tidak dipakai lagi)
  renderTable();
  renderPagination();
}
...
function renderTable() {
  ...
  tbody.innerHTML = pageData.map(r => {
    let cellsHtml = headers.map(h => {
      ...
      if (h === 'Status') {
        cell = `<td class="text-center"><span class="status done">done</span></td>`;
      }
